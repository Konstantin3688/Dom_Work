...
Console.WriteLine("Введите номер столбца 'с 0 столбца'");
int stolbProverki = int.Parse(Console.ReadLine());

if (strokaProverki <= strokaDlyaMassiva & stolbProverki <= stolbDlyaMassiva)
{
    Console.WriteLine("Вывод значения под адресом " + "[" + strokaProverki + "," + stolbProverki + "]: " + arr[strokaProverki, stolbProverki]);
}
else
{
